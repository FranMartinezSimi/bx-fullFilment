...
const SecondStep = ({ selectedItem, setSelectedItem }) => {
  const [active, setActive] = useState(false);
  useEffect(() => {
    setActive(selectedItem === 'secondStep');
  }, [selectedItem]);
  return (
    <CSSTransition
      in={active}
      ...
      unmountOnExit
    >
      <Card className="shadow" cardBackground="/onboarding-bg2.png">
        <ul className="d-flex flex-column justify-content-between mb-0" style={{ minHeight: '440px' }}>
          <li className="my-4 text-center">
            <div className={`${styles.cardImageContainer} m-auto mb-5`}>
              <img src="./onboarding-ico2.png" alt="imagen" width="320" />
            </div>
          </li>
          <li className="text-center">
            <h1 className="display-font">Revisa y carga tus órdenes</h1>
            <p style={{ fontSize: 16 }}>Conoce sus estados y síguelas con nuestro tracking</p>
          </li>
