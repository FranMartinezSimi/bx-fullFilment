...
const ThirdStep = ({ selectedItem, setSelectedItem }) => {
  const [active, setActive] = useState(false);
  useEffect(() => {
    setActive(selectedItem === 'thirdStep');
  }, [selectedItem]);
  return (
    <CSSTransition
      in={active}
      ...
      unmountOnExit
    >
      <Card className="shadow" cardBackground="/onboarding-bg3.png">
        <ul className="d-flex flex-column justify-content-between mb-0" style={{ minHeight: '440px' }}>
          <li className="my-4 text-center">
            <div className={`${styles.cardImageContainer} m-auto mb-5`}>
              <img src="./onboarding-ico3.png" alt="imagen" />
            </div>
          </li>
          <li className="text-center">
            <h1 className="display-font">Gestiona tu inventario</h1>
            <p style={{ fontSize: 16 }}>
